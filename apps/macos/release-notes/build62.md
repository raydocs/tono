...
- Fixes an upgrade case where a routing list already stored by an earlier version kept being read as unsigned, so any entry that version had not recognised stayed unused until the next update was published.
