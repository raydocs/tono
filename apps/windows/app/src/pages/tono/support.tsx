...
const SupportPage = () => {
  ...
  // TONO_DNS_POLICY_CONFLICT) ride in dnsLastError by design but are not failures — the
  // tunnel stays protected. Showing them under "last error" panics users for a healthy
  // connection, so they get their own row. The first two mirror DNS_WARNING_MARKERS in
  // src-tauri/src/tono/connection_health.rs. The policy conflict is the Service's separate
  // resolver_policy_warning, which only the diagnostics report folds into dnsLastError.
