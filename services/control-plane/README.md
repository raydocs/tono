...
- access token 是 24 小时 HS256 JWT；refresh token 仅以 SHA-256 hash 保存并原子轮换。长 JWT 只减少 refresh 的 D1 写入，不延迟撤销：每个受保护请求仍从 D1 重读 session、user 与 device。`POST /auth/logout` 必须带 access Bearer，可选正文 `{refreshToken}`，只撤销该 access token 对应 session（不会登出其他设备）。禁用用户或撤销/LRU 淘汰设备会立即使 access/refresh 鉴权失败。
...
- **`0037` 安全限制（#4）优先于上述 cutover 顺序**：有 legacy source 或非零 legacy last-seen 时，GET 返回 `legacy_handoff_boundary_unavailable`，POST 和数据库 trigger 都阻止切换。静默 1,800 秒不能证明配对账务边界，不能通过删除水位或清空 last-seen 绕过。保持现有 legacy collection；当前只允许无 legacy 历史的安装切换，不修改已处于 `v2_required` 的账务。完整无损 handoff 协议仍待 #4，不能声称已修复历史漏计。
