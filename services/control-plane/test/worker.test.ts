...
import automaticWindowsFixture from './fixtures/windows-automatic-diagnostics-v1.json';
...
`;
  ...
  it('keeps same-VPS Hysteria2 identity account-scoped without changing homeSocks5 routing', async () => {
    const yaml = `proxies:
  - name: "Synthetic Dual VPS"
    type: vless
    server: 8.8.4.4
    port: 443
    uuid: '{{TONO_CLIENT_UUID}}'
...
`;
    expect((await admin('exit-catalog', { yaml, expectedRevision: 0 }, 'PUT')).status).toBe(200);
    const owner = await createAccount('dual-home-owner');
    const other = await createAccount('dual-no-home');
    const home = await admin('home-exits', {
      proxyName: 'Synthetic Dual Home', displayName: 'Synthetic Home', kind: 'socks5',
      socks5Host: 'resi.example.com', socks5Port: 11080,
      socks5Username: 'synthetic-user', socks5Password: 'synthetic-home-secret',
    });
    expect(home.status).toBe(201);
    const homeId = (await home.json() as any).homeExit.id;
    expect((await admin(`users/${owner.user.id}/home-binding`, { homeExitId: homeId }, 'PUT')).status).toBe(201);
    const view = async (accessToken: string) => {
      const response = await api('exit-catalog', { headers: { authorization: `Bearer ${accessToken}` } });
      expect(response.status).toBe(200);
      return await response.json() as any;
    };
    const bound = await view(owner.accessToken);
    const unbound = await view(other.accessToken);
    expect(bound.routing.homeSocks5).toEqual({
      host: 'resi.example.com', port: 11080, username: 'synthetic-user', password: 'synthetic-home-secret',
    });
    expect(unbound.routing?.homeSocks5).toBeUndefined();
    const identities: string[] = [];
    for (const catalog of [bound, unbound]) {
      const identity = catalog.yaml.match(/^\s+uuid: '([^']+)'$/m)?.[1];
      expect(identity).toMatch(/^[0-9a-f-]{36}$/);
      identities.push(identity);
      expect(catalog.yaml).toContain(`password: '${identity}'`);
      expect(catalog.yaml).not.toContain('{{TONO_CLIENT_UUID}}');
      expect(catalog.sha256).toBe(await sha256(catalog.yaml));
      expect((catalog.yaml.match(/- name:/g) ?? []).length).toBe(1);
    }
    expect(identities[0]).not.toBe(identities[1]);
    await env.DB.prepare("UPDATE home_exits SET status = 'disabled' WHERE id = ?").bind(homeId).run();
    expect((await api('exit-catalog', { headers: { authorization: `Bearer ${owner.accessToken}` } })).status).toBe(503);
  });
...
`;
  ...
  it('accepts Windows automatic enum-only snapshots via the existing telemetry schema', async () => {
    // Generated by the Rust automatic_diagnostics synthetic context test, never a customer log.
    const account = await createAccount('automatic-windows-diagnostics');
    const response = await api('telemetry/windows', json({ window: automaticWindowsFixture }, account.accessToken));
    expect(response.status).toBe(201);
    const receipt = await response.json() as any;
    const stored = await env.DB.prepare('SELECT * FROM telemetry_windows WHERE id = ?').bind(receipt.id).first<any>();
    expect(stored.user_id).toBe(account.user.id);
    const payload = JSON.parse(stored.payload_json);
    expect(payload.events).toHaveLength(automaticWindowsFixture.eventCount);
    expect(payload.events).toEqual(expect.arrayContaining([
      expect.objectContaining({ kind: 'autoConnectionFailure', code: 'serviceUnavailable' }),
      expect.objectContaining({ kind: 'autoConnectionStep', stage: 'preparingService', outcome: 'failed', elapsedMs: 1234 }),
      expect.objectContaining({ kind: 'autoConnectionFact', code: 'remoteReachability', outcome: 'notObserved' }),
      expect.objectContaining({ kind: 'autoConnectionFact', code: 'captureConsistent', live: false }),
    ]));
    for (const event of payload.events) {
      expect(event.mode).toBe('notConnected');
      expect(event.reference).toMatch(/^[A-Za-z0-9_-]{16}$/);
      expect(event).not.toHaveProperty('error');
    }
    for (const privateValue of ['private-account', 'private-directory', 'private-credential', '192.0.2.1']) {
      expect(stored.payload_json).not.toContain(privateValue);
    }
    const listed = await admin(`telemetry/windows?userId=${account.user.id}`, undefined, 'GET');
    expect(listed.status).toBe(200);
    expect((await listed.json() as any).windows.some((window: any) => window.id === receipt.id)).toBe(true);
  });
