...
/**
 * 更新源, the same five rows the Worker's `releases/channels` returns. Only
 * macOS and Windows have a feed the Worker renders; the rest say so rather
 * than showing a path that answers nothing.
 */
  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use((req, res, next) => {
        ...
        if (parts[0] === 'releases' && parts[1] === 'channels' && parts.length === 2) {
