...
# was built to end.
#
# So the agreement itself is the thing under test, checked from outside all three.
...
ok "the deployment and macOS client declare the same signing key"
...
# --- the signed byte layout: three declarations, one value -------------------
...
# Swift escapes the newline once for its own source; the two JavaScript files
# write the same escape in a single-quoted string. All three should read
...
ok "all three sign over the same byte prefix"
...
expected_protected="anthropic.ai anthropic.com browser-intake-ap1-datadoghq.com browser-intake-ap2-datadoghq.com browser-intake-datadoghq.com browser-intake-datadoghq.eu browser-intake-ddog-gov.com browser-intake-us3-datadoghq.com browser-intake-us5-datadoghq.com cf-assets.www.cloudflare.com challenges.cloudflare.com clau.de claude.ai claude.app claude.com claude.site claudemcpclient.com claudemcpcontent.com claudestudio.com claudeusercontent.com cloudflareinsights.com datadoghq.com featuregates.org formulae.brew.sh growthbook.io hcaptcha.com link.com raw.githubusercontent.com registry.npmjs.org sentry.io servd-anthropic-website.b-cdn.net statsig.com statsigapi.net storage.googleapis.com stripe.com stripe.network stripecdn.com tono.app tono.com"
