...
- 归属：G1；Issue #601 条目「535R-C-F2 residual」「535R-C-F3 residual」「#579 residual」。App `Services/TonoAPIClient.swift`、
  `Services/Account/AccountSession+Runtime.swift`、`Services/Account/AccountSession+Telemetry.swift`、`Services/KillSwitchService.swift`；测试 `AccountSessionRequestTests.swift`、
...
- 缺陷修复：
  ...
  - 535R-C-F3：`startCloudOnlyRuntimeThrowing` 记下 `entitlementRefusals`，每个 await 之后和失败路径上都复查；期间来的拒绝
    ...
    `activateCloudFallback` 在目录读取之后同样复查，不为被拒绝的会话选出口。#653 复审 codex:F1（已确认，次要）：
    `fail` 自己的清理（`cancelManagedCatalogRefresh`、`descriptorConsumer(nil)`）也要等，期间来的拒绝会被随后的 `.error`
    覆盖。改后 `fail` 入口记下计数，设 `.error` 前复查，有新拒绝就保留 `.suspended`；清理照做，保护不变。
...
- 工程与测试：每个行为一个 XCTest。`testACancelledReadStopsWaitingForItsTokenRenewal`（401 后续期被扣住，取消读取须在
  ...
  `boundToAnotherUser("alice")`，`installIfNeeded` 须原样抛出）；复审后加 `testARefusalDuringFailureCleanupIsNotOverwrittenByThatFailure`
  （`fail` 撤回描述符时收到拒绝，状态须仍为 `.suspended`）。红分支只含测试。
- 验证：未在本地编译（MacBook 不是构建机）；以 PR 准确 head SHA 上的 macOS CI（`macos-26`）为准。红 run 36215649454
  （`64c5f41e`）三个测试均以断言失败（2 s 等待超时；`ready` ≠ `suspended`；得到 `installFailed`）。修复 `e25f887b` 的 PR CI
  36215807789 通过。复审修复的红提交 `3c04e5d5`（只加 `fail` 那个测试）与新 head 的 CI 推送时尚未出结果。
- 候选/发布：仅源码，无新候选。
- 剩余限制：登出仍会等在途续期结束再吊销（有意保留）。`cloudFallbackConsumer` 已发起的 Connect 若之后才遇到拒绝，
  仍由 `enterEntitlementBlock` 撤目录、停 Core 处理，未改。Home-US 路径（`startSidecar` 的 home 分支、`startRuntimeMonitor`
  自己的等待）未加复查（#653 复审 opus:F1，建议项）：`AppProfile.homeExitEnabled` 是编译期常量 `false`，当前构建走不到；
  重新启用 Home-US 前须补上。未实机。
