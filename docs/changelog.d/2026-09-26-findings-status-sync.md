...
- 记录同步：`docs/FINDINGS_LEDGER.md` 中 105 行 `in-PR` 所引 PR 已全部合入 main，改为 `fixed(<该修复落到 main 第一父链上的合并提交>)`（经合并列车进入的记列车合入 main 的提交）；D7、H8-F4 仅部分修复，保持原状态；
  ...
  由 open/in-PR 改为 fixed 并补 PR 链接；合入 main 时同步本批已合 PR：H16-O-F3/F4 与 TM-claude-1-native-update（#649，2ba08b9b）、H22-C-F1（#648，b0348ba7）、H16-O-F7 与 F515-1（#651，7c8dc6e0）、TW-R-boot（#650，81147f40）、535R-C-F2-R1/535R-C-F3-R1/H19-O-F3-R1（#653，c3267820）、R612-O5（#652，a1f2f340）、#191（#647，f1d55368）改为 fixed；H4-F3（#646 只修一项）改为 open；F520-1 保持 open（#648 只收窄交错，审查 0e6bcb0f 指出屏障读数与操作状态仍无一致性保证）。`fixed` 只表示源码进 main，不表示实机验收。未动：H2-F3（#352 仍开）、H19-* 与 OD-0924-* 等无 PR 链接的行。
