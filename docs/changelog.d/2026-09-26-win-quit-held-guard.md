...
- 缺陷修复：F520-1：退出/重启拒绝框只看本 App 的释放登记和 `/kill-switch/status`，另一用户 App 发起的释放、或本 App
  ...
  `ReleaseKillSwitch`、`StopCore`（状态标记不带其 `release_kill_switch` 选项）或 `StartCore`（启动中被显式释放取代时会解除）时归为 `ReleaseMayComplete`，
...
- 工程与测试：新增 `#[test]` `refusal_dialog_does_not_promise_protection_while_the_service_runs_a_mutation`（屏障在线、
  `active_operation = ReleaseKillSwitch`、本地无释放 → `ReleaseMayComplete`；换成 `StartCore` 仍为 `ReleaseMayComplete`，换成 `LockKillSwitch` → `Held`；
  红分支骨架忽略 `active_operation`，应按断言失败）；
  vitest `intro-seen flag > remembers intro in memory when localStorage cannot store it`。
- 复审续修：只把可能撤除屏障的操作（ReleaseKillSwitch、StopCore、StartCore）当作释放，逐个核对了 Service 各操作处理函数中的 release/transition_after_stop 调用；H22-C-F1 改回总账原行（无分片）。
...
- 剩余限制：`/status` 读不到时仍为「未确认」；`/status` 先取 kill switch 再取操作标记，释放恰在两次采样之间完成时会读成
  「屏障在线、无操作」而显示「仍受保护」（复审 codex:F1，未改快照设计）；StopCore 不带释放、StartCore 未被取代时也显示「可能仍在解除」，偏保守；内存标记只在本次会话有效，重启 App 且存储仍不可写时会再看到一次欢迎页。
