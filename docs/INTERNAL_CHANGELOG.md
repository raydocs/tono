...
- **缺陷修复**：
  ...
  - 审查返工 2：只有“对端映像不是已注册的 `Tono.exe`”返回 401 `UnauthorizedOwner`；其余无法
    ...
    未完成）返回新错误码 `AppIdentityUnproven`（1014，HTTP 503），消息提示重试，若持续则修复 Tono
    ...
    管理员 `--emergency-disarm`。1013 在 main 上已被 `StaleReleaseEpoch` 占用，故用 1014。
- **新增/优化**：无。
- **工程与测试**：
...
- **剩余限制**：未实机验证；需上述 `icacls` 证据后才能合并，若设备 ACL 与模型不符（例如所有者是
  ...
  1014 做专门重试，只显示 Service 的消息。每次进入生命周期都会对 Tono.exe 做摘要并遍历安装树，耗时未实测，也未按
