...
- **续记 2（2026-09-24，Codex 第二轮核验 b6882589；Anthropic 逐条核实）**：F1、H3-F1、I6 进程内路径
  判为已修。以下三项确认并修复。
  - R607-F2 续项（确认）：`Unchanged` 分支不写盘，却仍按“当前磁盘文件”摘要重新盖章，磁盘被换成
    另一账户的有效缓存时会被记成本会话的。标记改记服务器本次返回内容的 YAML 摘要与 routing 摘要
    （routing 不在 YAML 摘要内）；仅当磁盘缓存的两项摘要与本次返回一致才写，否则删除已有标记。
    准入时用磁盘缓存重算两项并比对。
  - R607-F3（确认）：周期同步 401 → Suspended 只在内存；重启且控制面不可达时旧 token + 缓存仍匹配
    标记，恢复成 Ready。现在明确拒绝时删除标记：周期同步 401（`note_session_rejected`，先删标记再
    Suspended）、目录同步 403、启动恢复 401/403、`me` 返回 suspended。删除失败时内存标志
    `session_catalog_revoked` 在本进程内继续拒绝离线准入、阻止重新盖章并在每次判定时重试删除，
    新登录清除。保护（WFP）不变。未覆盖：策略同步单独收到 403（目录同步会先遇到同一拒绝）。
  - #583 续项（确认，可用性）：首选系统 DNS 请求被外层恢复截止时间取消时 `prefer_resolved` 不清除，
    重试复用同一 client，黑洞解析器会一直挡在健康 pins 之前。改为租约式：首选尝试期间持有偏好，
    未得到答复（失败或被取消）即在 drop 时清除；首选尝试改用 10 s 连接预算的独立 client
    （原为 30 s）。POST 送达规则不变。
  - 测试仍为三个（规则 5 上限四个），在原测试内扩展：restore 测试增加“Unchanged + 磁盘被替换 →
    不写标记且离线拒绝”和“401 → 标记被删，清空内存标志模拟重启后仍拒绝”（b6882589 上前者会写出
    A 的 token + B 的文件摘要、后者标记仍在，均失败）；transport 测试增加“首选尝试被取消后下一次
    请求 5 s 内由 pins 答复”（b6882589 上偏好保留，下一次先等 30 s 黑洞连接，失败）。均按代码推理，
    未实跑；Rust 仍交 windows-2025 CI。本轮未改前端。
  - 剩余：标记在 token 轮换后到下一次成功同步前会过期（300 s 只是调度间隔，失败、睡眠可更久），
    期间离线启动走 error；删除标记失败且进程退出后，下次启动仍可能匹配（文件系统删不掉时无别的持久
    手段）。
