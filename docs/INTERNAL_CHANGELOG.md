...
- **归属/来源**：G2 客户端账户状态与保护边界；macOS `AccountSession+Telemetry.swift`。
  内部审查 H17-O-F1、H17-O-F2（macOS 部分），与另一审查方 H17-G-F3 第 2、4 步为同一路径；
  Issue #510。基线 main bb2ed4e4 → 分支 `fix/macos-auth-401-keeps-protection-20260924`；
  提交时未合 main。
- **缺陷修复**：套餐到期、流量用尽、账户停用、设备吊销（含别处登录挤掉）时，Worker 对鉴权
  ...
  - 启动 restore 仍登出，但 PF/DNS 保持原样，并保留崩溃恢复的 resume 意图。界面落到登录页
    已有的"恢复网络"入口，与无存储会话的启动路径相同；重新登录后按原意图自动重连。
  - 后台上传的 401 改走 `refreshAccount`，与目录循环一样进入 `.suspended`。保护不变，显示
    原因页。
- **新增/优化**：无。
- **工程与测试**：`AccountSessionRequestTests` 新增两个 XCTest：
  - `testBackgroundUploadRefusedSessionSuspendsAndKeepsProtection`：遥测与 refresh 都回
    401；断言 disarm consumer 未被调用、状态为 `.suspended`、账户仍在。
  ...
  只含测试的提交 a1154ced（产品代码未改）在 GitHub-hosted macOS CI run 35973008589 上实际
  跑红：build 作业只有这两个测试失败（`AccountSessionRequestTests` 51 项、6 处断言失败）。
  后台用例：disarm 被调用，状态为 `signedOut`；启动用例：disarm 被调用，resume 意图被清。
...
- **剩余限制**：
  - Worker 仍对所有不合格情形回 401（加 entitlement 码是另一单元）。因此启动时的 401 仍会
    登出，也不显示原因。
  - `.suspended` 不停止正在运行的 Core，不作废缓存的出口凭据（H17-C-F3）。
  ...
  - Windows 启动时的 401 仍会释放 WFP（`restore.rs:318-325`），不在本条范围。
