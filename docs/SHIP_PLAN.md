...
定稿日期：2026-09-10。基线：`main` `2cef4eac`（PR #140，第十四次控制面部署）。源码版本（当时）macOS / Windows **0.0.72**，现已升到 0.0.74（§2.3）。客户更新源仍是 Sparkle **0.0.67**、`windows/latest.json` **0.0.34**。GitHub 上的 `v0.0.72` / `tono-macos-0.0.72-build72` **不是**客户频道（issue #80）。

**这一发的产品版本号是 0.0.74（macOS build 74）。** 0.0.72 的 GitHub 标签已经存在且未进更新源；`tono-macos-0.0.73-build73` 指向旧源码 `bdc75a4e`，`v0.0.73` 是旧草稿，两者都不移动、不复用（老板 2026-09-26 定为 74）。标签在发布时才打，发的是已验收的那份产物（[RELEASE_LINES](RELEASE_LINES.md#customer-publish-g4)）。
...
继续排查确认：两端损坏/过期的更新日记会隐藏「更新未完成」提示，现已补文件回归与状态判断修复。
见[更新证据可见性](reports/UPDATE_EVIDENCE_2026-09-13.md)；#26 的安装器身份绑定、保护交接与真机验收仍未完成，不据此关闭 G3。

安装器续修：日记校验/持久化拒绝须在替换入口中止，NSIS 不得把这类拒绝当普通错误自动重试。
见[安装器日记门禁](reports/INSTALLER_JOURNAL_GATE_2026-09-13.md)；旧多用户发现路径尚未替换为 Service 身份回执，仍不关闭 #26/G3。
...
- 背景：源码已修（`PluginBuilder::new("tono-plugin-core")`），09-08 实机 FAIL 是旧包。`docs/WINDOWS_0_0_72_DEVICE_ACCEPTANCE.md` 仍是 qualification incomplete。含 Encrypted DNS + 连接中 pill + TUN DNS 的包是 run [`34574027022`](https://github.com/raydocs/tono/actions/runs/34574027022) SHA `ae2e699c…`（§8）。§9（`7bc7aaf9…`）有备用 UDP 栏但仍 REJECT 全部 UDP。**测 hy2 用 §10：** run [`34599676962`](https://github.com/raydocs/tono/actions/runs/34599676962)，installer SHA-256 `2af3f3b1894fd5b5d12b9507df723d0f8a87715d91fd1b1bb4fb8809d31ba674`（源 `3abe64b6`，选中 hy2 时不再 REJECT UDP）。不签名、不推更新源、版本仍 0.0.72。§7 旧包大按钮写「取消」，不要再装。**实机未装 §8/§9/§10，G1.1 未过。**
- 步骤：Tono **先断开**。测 hy2 / 备用 UDP 用 §10 的 `gh run download` 或 nightly.link；只测 Encrypted DNS 也可用 §8。核 SHA-256，再装。连接；Win10 必须过 `securingDNS`；仪表盘速率与 Activity 离开 controller retry。断开收回 DNS。生产 Worker 已是 `7c38521c`；hy2 行要等目录 `--append` 之后刷新才看得到。
...
及同源夹具，并推进[特权事务原生接入](UPDATE_INTEGRATION_V1.md)：macOS 使用
Tono 自有完整 App 安装器，不再以 Sparkle continuation 充当安装证明；Windows
由 Service 和独立 SYSTEM 执行器持有安装权限。两端使用同一源码、版本及签名
releaseSequence。源码集成、普通 CI 和候选包都不等于 G3.3 真机通过；#26 保持开放。

下列 G3.1/G3.2 保留 legacy journal 的整改历史，不是 v1 的授权来源或接入方案。
旧日记不能迁移成可信收据，也不能继续用跨用户扫描或版本号相等代替安装器身份。
老板允许旧版手动换新；首次 v1 引导必须先正常、已验证地断开，再手动安装带
可信序列下限的新版。此步骤不能算作受保护自动更新成功。
...
- 两端先安装具备 v1 能力、可信序列下限的引导版本，再用同一新序列、同一源码
  的成对签名包测试受保护更新；不得回退 legacy 安装器，也不得为测试跳过签名。
- Windows：在 Windows 11 上验证 Service/WFP 静默交接、独立执行器替换、新进程
  身份及保护恢复。安装消费后中断一次，失败证据、回滚资源和序列高水位必须保留。
- macOS：验证 Developer ID 完整包、Helper/PF/DNS 交接、launchd 独立执行器、
  新进程恢复及中断回滚；`verify-release-gate.sh` 通过。首次 Helper 授权要求须说明。
- 验收：两份短笔录（实际包哈希、源码/序列、特权回执与执行事实、DNS/包级漏流量
  观察、成功和失败场景）。托管 CI 不能替代安装机；#26 在证据贴上之前保持 OPEN。
...
- 升版本号（§2.3）。写 `apps/macos/release-notes/build74.md` 与 `apps/windows/release-notes/0.0.74.md`，中英或中文与现有系列一致。
- 发布说明必须包含：已知局限（无手机/Linux；Windows 可能 SmartScreen；hy2 自动或仅手动，引用 `transport-hy2.md`；睡眠恢复若未认证就写未认证）。**不要**承诺银行 3DS / 全部 Secure DNS。
- 跑 `verify-desktop-version.py --expected 0.0.74` 与稳定性脚本。
...
- 验收：未改过 hosts 的内部机器「检查更新」看到 0.0.74；`curl` 更新源版本字符串是 0.0.74 不是 0.0.72。
