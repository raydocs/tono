...
- 杭州 `47.110.84.71` 只出站：东京 VLESS TCP 通；五台 Dedirock hy2（Niagara / Erie / Sunset / Mesa / Grove）UDP 握手均为 EXIT 0 且经 hy2 到 Google 通。Panstar 东京入站 UDP 被商家拦住（工单 #529）。自动切换默认关。客户目录默认剥掉 ` · hy2`（`HY2_CATALOG_EMAILS` 或 `X-Tono-Accept: hy2`）。**生产目录仍不塞块**（须先部署带该过滤的 Worker）。本分支节点页顶部有独立「备用 UDP」栏。
