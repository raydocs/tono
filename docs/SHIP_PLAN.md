...
- **G2 手选备用通道：** 握手 eof / `CORE_EXIT_UNREACHABLE` 时，失败卡片、托盘、菜单栏、非首页横幅在目录有 ` · hy2` 时提供「试用备用通道」（先同城；没有则其它城。东京 UDP 被商家拦，大陆下一手是 Dedirock）。用户点击才切到 hy2 并重试。G2.8 自动切换仍关。生产目录仍不塞块。
