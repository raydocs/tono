...
- 杭州 `47.110.84.71` 只出站：东京 VLESS TCP 通；五台 Dedirock hy2（Niagara / Erie / Sunset / Mesa / Grove）UDP 握手均为 EXIT 0 且经 hy2 到 Google 通。Panstar 东京入站 UDP 被商家拦住（工单 #529）。自动切换默认关。客户目录默认剥掉 ` · hy2`（`HY2_CATALOG_EMAILS` 或 `X-Tono-Accept: hy2`）。**生产目录仍不塞块。** 生产 Worker 仍是 `main` `2cef4eac`。Worker-only PR **#145** 相对 `main` 只有控制面（hy2 合同 + 过滤 + 0072）以及合入后用的 `write-dedirock-hy2-catalog-sources.rb`；合 #145 后老板跑 `deploy-control-plane-main.sh`，不必等本分支 G1 客户端。先 PUT 会打挂旧包。本分支节点页顶部有独立「备用 UDP」栏。
