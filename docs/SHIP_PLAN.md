...
- **G2 手选备用通道：** 握手 eof / `CORE_EXIT_UNREACHABLE` 时，失败卡片、托盘、菜单栏、非首页横幅在目录有 ` · hy2` 时提供「试用备用通道」（先同城；没有则其它城。东京 UDP 被商家拦，大陆下一手是 Dedirock）。用户点击才切到 hy2 并重试。未连接时在设置页/托盘选城（含 hy2）会真正发起连接，不再只改选择。macOS 不再在 `CORE_EXIT_UNREACHABLE` 上自动换城（与 Windows 一致：国内每个城市都是同一种 TLS close，换城只会跳选择器）。G2.8 自动切换仍关。生产目录仍不塞块。
