...
| `PATCH ledger/{id}` | `LedgerEntryDto`。只接受 `{ note?, paidAt?, subjectType?, subjectId? }`，不能改 `currency`；月已锁定 409 `MONTH_CLOSED`；已冲正的原行与冲正行不能改 `subjectType`/`subjectId`（409 `ALREADY_REVERSED`，`note`/`paidAt` 仍可改） |
...
| `GET months/{month}` | `MonthSummaryDto`。收入 = Σ(revenue+credit)−Σ(refund)；成本 = Σ cost；客户成本 = 名下 Claude/ChatGPT 账号成本 + 该月该节点/线路字节占比摊到的 server/home_line 成本。用量缺测（含连接过但没有字节记录，此时成本无法分摊）或有字节无成本时 `pending: true` 且客户 `marginCnyMinor` 为 null。已关账时 `frozen: true`、`frozenAt` 为关账时刻，`revenueCnyMinor`/`costCnyMinor`/`marginCnyMinor`/`unreconciled` 取 `ops_month_close` 冻结值；`customers[]`/`nodes[]` 取关账时写入的 `summary_json`，不再现算。快照缺失（关账早于该列）或超 65536 字节只存了 `{ customers: [], nodes: [], partial: true }` 时现算这两列并带 `frozenPartial: true`。未关账 `frozen: false`、`frozenAt: null`。`billsWithoutLedger` = 本月应有账单（有价格的在役节点 / 月付家宽 / 在用 Claude·ChatGPT 账号）但没有 `cost` 台账；`ledgerWithoutBill` = 有 `cost` 台账但对象不存在 / 已退役 / 没登记价格；按 `(subjectType, subjectId)` 配对，冲正对冲后计；`fleet` 类目不进对账。`unreconciledBills` 是两边行数之和，与按用量算的 `unreconciled` 不是一个数 |
| `POST months/{month}/close` | `MonthSummaryDto`。body `{ notes? }`。已锁定 409 `MONTH_CLOSED`，写入当时的四个总数，并把 `customers[]`/`nodes[]` 写入 `summary_json`（超 65536 字节则存 `{ customers: [], nodes: [], partial: true }`） |
| `GET months/{month}/export.csv` | `text/csv; charset=utf-8`，UTF-8 BOM，一行一笔 + 合计行。合计 CNY 与同币种原币合计均为收入+贷记−退款−成本；多币种时金额列留空。以 `= + @`、tab、CR 开头，或 `-` 后跟非数字的单元格前加 `'` 并加引号；负数金额不加前缀。不是 JSON，不进 GET 检查器表 |
