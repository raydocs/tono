...
| --- | --- | --- | --- |
| `POST /api/v1/telemetry/windows` | User access token (`auth()`) | 72 KiB (`TELEMETRY_BODY_MAX_BYTES`) | `RATE_LIMIT_TELEMETRY_USER_HOUR` 6 / 3600; `RATE_LIMIT_TELEMETRY_USER_DAY` 80 / 86400. User-keyed only: a connected client reaches the Worker through its exit node, so an IP bucket would make every user on that node share one budget. Code defaults; not set in `wrangler.jsonc`. |
| `POST /api/v1/telemetry/failures` | User access token (`auth()`) | 8 KiB | `RATE_LIMIT_FAILURE_USER_HOUR` 12 / 3600; `RATE_LIMIT_FAILURE_USER_DAY` 60 / 86400. User-keyed only (same reason). Limiter runs **before** the body is read. Code defaults; not set in `wrangler.jsonc`. |
...
| `POST /api/v1/diagnostics` (reports; extra row so every `RATE_LIMIT_*` key has a home) | User access token (`auth()`) | 32 KiB (`DIAGNOSTICS_BODY_MAX_BYTES`) | `RATE_LIMIT_DIAGNOSTICS_USER_HOUR` 5 / 3600; `RATE_LIMIT_DIAGNOSTICS_USER_DAY` 20 / 86400. User-keyed only (same reason). Production `wrangler.jsonc` sets the same values. |
